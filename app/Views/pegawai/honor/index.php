...
<?= $this->section('judulbesarcontent') ?>
    <h1 class="h3 mb-2 text-gray-800">Pegawai Honor</h1>
<?= $this->endSection() ?>

<?= $this->section('content') ?>
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Data Pegawai Honor</h6>
    </div>
    <div class="card-body">
    <div class="table-responsive">
    <?php if(session()->get( 'level' ) == 1 || session()->get( 'level' ) == 2) {?>
        <button type="button" class="btn btn-sm btn-primary" id="btnTambah" data-toggle="modal" data-target="#exampleModal"><i class="fa fa-plus"></i> Data Honor </button>
        <br><br>
    <?php } ?>
    <?php if(session()->get( 'level' ) == 1 || session()->get( 'level' ) == 2) {?>
        <table class="table table-bordered" id="dataTableHonor" width="100%" cellspacing="0">
    <?php } else { ?>
        <table class="table table-bordered" id="dataTableHonor1" width="100%" cellspacing="0">
    <?php } ?>
            <thead>
                <tr>
                    <th>No</th>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Honor</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form  action="<?php echo base_url('honor/savedata') ?>" method="post" id="tambahData" enctype="multipart/form-data">
...
<div class="modal fade" id="exampleModal1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Ubah Data Honor</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form  action="<?php echo base_url('honor/updateData') ?>" method="post" id="ubahData" enctype="multipart/form-data">
                <div class="modal-body">
...
<?= $this->endSection() ?>
<?= $this->section('script') ?>
    <script src="<?= base_url(); ?>/js/honor.js"></script>
<?= $this->endSection() ?>
